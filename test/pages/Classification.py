...
r_data_ = st.file_uploader('Upload Classification Data')
...
if r_data_ is not None:
    ...
    with st.expander("Start"):
        missing_cols,missing_series = AC.search_missing_value(r_data)
        ...
        analysis_data = AC.interpolation(r_data, missing_cols,
                                    method_box) 
...
if analysis_data is not None:
    ...
    with st.expander("Start"):      
        ...
        option = st.selectbox('Select Variable', te_data.columns)
...
try:
    if setting_result is not None:
        ...
        with st.expander("Start"):
            ...
            single_visual_graph = AC.single_visual(single_result.iloc[0:10])
